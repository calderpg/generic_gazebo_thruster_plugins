...
namespace gazebo
{
  class GenericGazeboForceThrusterPlugin : public ModelPlugin
  {
  protected:
    ...
    ignition::math::Vector3d thrust_direction_;
    ...
    void CommandMessageCallback(const std_msgs::Float64ConstPtr& msg)
    {
      ...
      {
        ROS_DEBUG(
            "[Gazebo force thruster plugin: %s] Commanded throttle below "
      ...
      {
        ROS_WARN(
            "[Gazebo force thruster plugin: %s] Commanded throttle %f below min"
            " throttle, throttling to min throttle %f", thruster_name_.c_str(),
            std::abs(commanded_throttle), min_throttle_);
        throttle_ = min_throttle_;
      }
      else if (std::abs(commanded_throttle) > max_throttle_)
      {
        ROS_WARN(
            "[Gazebo force thruster plugin: %s] Commanded throttle %f above max"
            " throttle, throttling to max throttle %f", thruster_name_.c_str(),
            std::abs(commanded_throttle), max_throttle_);
        throttle_ = max_throttle_;
      }
      else
      {
        ROS_DEBUG("[Gazebo force thruster plugin: %s] Commanded throttle to %f",
  ...
  public:
    ...
    GenericGazeboForceThrusterPlugin()
        : thrust_direction_(ignition::math::Vector3d::UnitZ),
          rated_thrust_(0.0), min_throttle_(0.0), max_throttle_(0.0),
          throttle_(0.0) {}

    virtual ~GenericGazeboForceThrusterPlugin()
    {
      if (ros_node_ptr_)
      {
    ...
    virtual void Load(physics::ModelPtr model_ptr, sdf::ElementPtr sdf_ptr)
    {
      ...
      if (sdf_ptr->HasElement("body_thrust_direction"))
      {
        ignition::math::Vector3d thrust_direction =
            sdf_ptr->Get<ignition::math::Vector3d>("body_thrust_direction");
        const double mag = thrust_direction.Length();
        if (mag > 0.0 && !std::isnan(mag))
        {
          thrust_direction_ = thrust_direction.Normalize();
        }
        else
        {
          throw std::invalid_argument(
              "Invalid body_thrust_direction parameter provided");
        }
      }
      else
      {
        throw std::invalid_argument(
            "No body_thrust_direction parameter provided");
      }
      // Listen to the update event.
      // This event is broadcast every simulation iteration.
      simulation_step_connection_ =
          event::Events::ConnectWorldUpdateBegin(
              boost::bind(&GenericGazeboForceThrusterPlugin::SimulationStep,
                          this, _1));
      ...
      ros::SubscribeOptions subscribe_options =
          ros::SubscribeOptions::create<std_msgs::Float64>(
              ...
              + "/throttle/command", 1,
              boost::bind(
                  &GenericGazeboForceThrusterPlugin::CommandMessageCallback,
                  this, _1),
      ...
      ros_callback_thread_ =
          std::thread(std::bind(
              &GenericGazeboForceThrusterPlugin::ROSCallbackThread, this));
    }
    ...
    void SimulationStep(const common::UpdateInfo&)
    {
      ...
      const double thrust_force = rated_thrust_ * throttle_;
      const ignition::math::Vector3d body_frame_thrust_vector =
          thrust_direction_ * thrust_force;
      // Apply the thrust force
      const ignition::math::Pose3d thruster_body_pose =
          thruster_body_->WorldCoGPose();
      const ignition::math::Vector3d world_frame_thrust_vector =
          thruster_body_pose.Rot().RotateVector(body_frame_thrust_vector);
      thruster_body_->AddForce(world_frame_thrust_vector);
      // Update the status
      ...
      thrust_msg.wrench.force.x = body_frame_thrust_vector.X();
      thrust_msg.wrench.force.y = body_frame_thrust_vector.Y();
      thrust_msg.wrench.force.z = body_frame_thrust_vector.Z();
      thrust_msg.wrench.torque.x = 0.0;
      thrust_msg.wrench.torque.y = 0.0;
      thrust_msg.wrench.torque.z = 0.0;
      ros_thrust_status_pub_.publish(thrust_msg);
    }
  };

  // Tell Gazebo about this plugin, so that Gazebo can call Load on this plugin.
  GZ_REGISTER_MODEL_PLUGIN(GenericGazeboForceThrusterPlugin)
}
